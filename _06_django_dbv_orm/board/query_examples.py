...
def compare_author_queries():
    # 기본 방식, 최적화 방식 비교
    # - 두 측정은 별도의 QuerySet을 만들어서 서로 값을 공유하지 않는 상태를 만든다.
    ...
    plain = _measure(
        ...
        #   -> 이때, 조회 방식은 Post의 각 행에 접근하며 Author의 pk 값을 얻어와 Author 테이블에서 select 1회 시도
    ...
    optimized = _measure(
        ...
        # - Post와 Author 테이블을 JOIN 해서 SELECT를 1회만 실행
...
def compare_comment_queries():
    ...
    optimized = _measure(
        ...
        #   두 조회 결과를 장고 서버 메모리에서 연결하는 작업 수행
...
# Post별로 Comment가 몇 개 작성되어 있는지 숫자 조회
